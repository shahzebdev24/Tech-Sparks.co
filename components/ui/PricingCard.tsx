...
export function PricingCard({ plan, className = '', align = 'center', delay = 0 }: PricingCardProps) {
  ...
  return (
    ...
    <Card 
        ...
        className={`flex flex-col h-full border-white/5 transition-all duration-500 group relative !overflow-visible ${
            isHighlighted ? 'scale-105 z-10 border-indigo-500/30' : 'hover:border-white/10'
    ...
    >
        ...
        <Heading level={5} className={`text-white font-bold mb-4 group-hover:text-indigo-400 transition-colors uppercase tracking-widest text-[10px] ${isCenter ? 'mb-6' : ''}`}>
            {plan.name}
        </Heading>
        ...
        {plan.description && (
            <Text className={`!text-white/80 text-sm font-semibold leading-relaxed mb-8 ${isCenter ? '' : 'text-left'}`}>
                {plan.description}
            </Text>
        )}

        <div className={`space-y-4 mb-12 w-full ${!isCenter ? 'flex-1 border-t border-white/5 pt-8' : ''}`}>
            {plan.features.map((feature, index) => (
                <div key={index} className="flex items-center gap-3">
                    ...
                    <span className="text-gray-300 text-sm font-semibold leading-tight text-left">{feature}</span>
                </div>
            ))}
        </div>
        ...
        <div className="mt-auto w-full">
            <Button 
                ...
                fullWidth 
                href={href}
                className={isHighlighted ? 'glossy-button' : 'border-white/10 text-white hover:bg-white/5'}
